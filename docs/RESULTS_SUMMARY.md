...
8 个位置是 C3k2 模块索引 2/4/6/8/13/16/19/22，其中 Backbone 4 处、融合与检测尺度路径 4 处。三种中间配置均通过 smoke 后独立重训练：

| 配置 | 数量 | P | R | mAP@0.5 | mAP@0.5:0.95 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 无 LECA | 0 | .9142 | .8452 | .9489 | .5848 |
| 仅 Backbone | 4 | .8701 | .8644 | .9280 | .5232 |
| 仅融合路径 | 4 | .9483 | .8284 | .9372 | .5888 |
| 仅 P3/P4/P5 | 3 | .9386 | .8774 | .9455 | .6083 |
| Full | 8 | **.9742** | **.9226** | **.9796** | **.6140** |

**已观察**：Full 四项最高；P3/P4/P5 是最强精简方案。Backbone-only 与 Fusion-only 参数相同但指标取向不同，支持位置而非参数数量影响结果。当前尚未逐点完成 8 次 leave-one-out，因此结论是“全覆盖配置综合最优”，不是每个单点已被独立证明不可删除。

## 收敛模型的逐层实际因子

使用 Full LECA 最优权重遍历全部 146 张 Hard Test，统计真正乘到特征上的动态因子。相对内部 ECA 基础权重，`w_stat=w_sup×w_rec×w_bri` 在 8 个位置的均值依次为：

`.9327 / 1.0544 / 1.0730 / .9603 / .9151 / .9321 / .9699 / 1.0034`。

这对应“第 2 层抑制—第 4/6 层增强—第 8 层及融合后抑制—第 22 层近中性”的层级增益曲线。方差分支是幅度最大的校准来源；低响应分支在第 4/6 层约增强 3.1%/3.3%，在第 13/16 层约抑制 1.3%/1.7%；`w_bri` 的层均值范围为 `.9832–1.0296`，自然图像上的样本间变化较小，因此主要表现为层级共享增益。

在同一收敛模型上进行推理时中性化，Full/关闭 Var/关闭 Rec/关闭 Bri 的 mAP@0.5:0.95 分别为 `.6140/.6082/.6119/.6125`。关闭 Var 同时提高了 P/R/mAP@0.5，说明该分支改变了宽松检出与严格定位的平衡。该实验只表示当前收敛点的局部敏感性，不替代独立重训练消融。

完整逐层分位数见 `reports/leca_layer_factor_summary_seed42.csv`；中性化结果见 `reports/hard_branch_neutralization_seed42.csv`。可视化保存在本地 ignored 目录 `artifacts/visualizations/leca_layer_factors/`。
