...
from .attention import ECA,LECA,LUECA,AdaptiveULECA,CascadeULECA
...
# Paper: "SimAM: A Simple, Parameter-Free Attention Module for CNNs" (ICML 2021)
# ============================================
...
class SimAM(nn.Module):
    ...
    完全无参数的注意力机制，基于神经科学中的能量函数
    非常适合轻量级模型
    
    论文: https://proceedings.mlr.press/v139/yang21o.html
    """
...
class C2f_CoordAtt(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv2 = nn.Conv2d((2 + n) * self.c, c2, 1)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        self.attention = CoordinateAttention(c2, c2)  # 添加CoordAtt
...
class NAM(nn.Module):
    def __init__(self, channels):
        ...
        self.Spatial_Att = Spatial_Att(channels)
...
class SE(nn.Module):
    def __init__(self, c1, ratio=16):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(c1, c_hidden, 1, padding=0, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(c_hidden, c1, 1, padding=0, bias=False),
            nn.Sigmoid()
        )
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        # 使用卷积层代替全连接层，减少参数并保持空间维度（虽然这里是1x1）
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        # 在通道维度上做 MaxPool 和 AvgPool -> [B, 1, H, W]
