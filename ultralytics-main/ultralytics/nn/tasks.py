...
from ultralytics.nn.modules import (
    ...
    C2,
    C2PSA,
    C2f_SimAM, C2f_CoordAtt,EMA,NAM,SE,CBAM,LECA,
    C3,
    C3TR,
    ELAN1,
    OBB,
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (C2f_SimAM, C2f_CoordAtt):
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            args.insert(2, n)
            args.insert(4, 1)  # 关键修复：插入 g=1
            n = 1
        ...
        # ================== 新增代码开始 ==================
            ...
            c2 = ch[f]  # 输出通道 = 输入通道
            args = [c2] # 强制只传一个参数给 EMA(channels)，忽略 YAML 里的多余参数
        # ================== 新增代码结束 ==================
        # ================== 新增 NAM 代码 ==================
        elif m is NAM:
            c2 = ch[f]
            args = [c2]  # 确保只传一个 channels 参数给 NAM
        # ==================================================
        # ================== 新增 SE 代码 ==================
        elif m is LECA:
            c2 = ch[f]
            args = [c2] # 默认 reduction=16，如果想改 reduction 可以在这里魔改
        elif m is SE:
            c2 = ch[f]
            args = [c2] # 默认 ratio=16，如果想改 ratio 可以在这里魔改
    # ==================================================
        # ================== 新增 CBAM 代码 ==================
        elif m is CBAM:
            c2 = ch[f]
            args = [c2] # 默认 kernel_size=7，如果需要改可以 args = [c2, 7]
        # ==================================================
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            c1 = [ch[x] for x in f]  # 1. c1 是一个通道列表, e.g., [256, 512, 1024]
            c2 = args[1]  # 2. c2 是你的输出通道 (256)
            args = [c1, *args[1:]]  # 3. 重组 args, 传递给 BiFPN 的 __init__

        elif m in frozenset(
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            if f == -1 and isinstance(layers[-1], BiFPN):
                args.append([ch[-1]] * 3)  # 告诉 Detect：从 BiFPN 接收 3 个输出
            else:
                args.append([ch[x] for x in f])  # 原始逻辑
